...
void TextRenderer::init()
{
    ...
    for (unsigned char c = 0; c < 128; c++)
    {
        ...
        {
            LOG_ERROR << "ERROR::FREETYTPE: Failed to load Glyph " << c;
            continue;
        }
        // generate texture
...
void TextRenderer::RenderText(Camera2D& cam, std::string text, float x, float y, float scale, glm::vec3 color)
{
    // activate corresponding render state
    x = x - cam.viewportWidth / 2.0f;
    y = y - cam.viewportHeight / 2.0f;
	auto pMatrix = cam.getProjectionMatrix();
